...
## 企業概要の別経路について（`edinet_server.py`、任意・ローカル専用）

以前のバージョンでは、公式EDINET API（金融庁）から都度企業概要を取得するローカルプロキシ（`edinet_server.py`）を使っていました。現在は EDINET DB のデータセットに事業概要・AI所見が同梱されているため、**本番サイト（Cloudflare Workers）ではこの機能は使われません**（`/api/edinet/overview`はバックエンドを持たない静的サイトでは常に失敗し、パネルには何も表示されないだけで他の機能に影響はありません）。ローカルで公式EDINET側の一次情報を見比べたい場合のみ、`python edinet_server.py 8010` を使うと`serve_nocache.py`の代わりに動作します（`EDINET_API_KEY`環境変数が必要）。
