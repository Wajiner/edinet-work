...
(function () {
  ...
  let companies = [];
  let datasetMeta = DEFAULT_DATASET_META; // {type:'default'|'imported', downloadedAt, ...}
  ...
  function updateDatasetBadges() {
    ...
    if (datasetMeta.type === 'imported') {
      ...
      providerNote.textContent = '出所: J-Quants API（東証プライム上場30社）';
    }
  ...
  function updateDatasetInfoBox() {
    ...
    if (datasetMeta.type === 'imported') {
      ...
      el.textContent = `既定の実データセットを使用中です（${companies.length}社、東証プライム）。`;
    }
  }
  ...
  function initDatasetUI() {
    ...
    document.getElementById('jq-import-file').addEventListener('change', async (e) => {
      ...
      if (!file) return;
      try {
        const { companies: importedCompanies, meta, fiscalYears } = await DatasetStore.importFromFile(file);
        ...
        DatasetStore.saveToLocalStorage(importedCompanies, newMeta, fiscalYears);
    ...
    document.getElementById('jq-reset-sample-btn').addEventListener('click', () => {
      DatasetStore.clearLocalStorage();
  ...
  function bindEvents() {
    ...
    document.getElementById('market-select').addEventListener('change', (e) => { state.market = e.target.value; });
  ...
  function init() {
    ...
    // 起動時：JSON読込済みのキャッシュがあれば復元、なければ空の状態で開始
    const cached = DatasetStore.loadFromLocalStorage();
    if (cached) {
      setDataset(cached.companies, cached.meta);
    } else {
      setDataset([], DEFAULT_DATASET_META);
    }
  }
